...
<?php
	...
	if(isset($_SESSION['user'])){
		...
		try{
			$total = 0;
			$stmt = $conn->prepare("SELECT *, cart.id AS cartid, cart.quantity AS quan FROM cart LEFT JOIN products ON products.id=cart.product_id WHERE user_id=:user");
			$stmt->execute(['user'=>$user['id']]);
			foreach($stmt as $row){
				$stmt2 = $conn->prepare("SELECT *, category.cat_slug AS catslug, products.id AS prodid FROM products LEFT JOIN category ON category.id=products.category_id WHERE products.id = :product_id");
				$stmt2->execute(['product_id' => $row['product_id']]);
				$row2 = $stmt2->fetch();
	
				$folder = $row2['catslug'];
				$image = (!empty($row2['photo'])) ? url_for('/images') . '/' . $folder . '/' .$row2['photo'] : url_for('/images/noimage.jpg');

				$subtotal = $row['price']*$row['quan'];
				$total += $subtotal;
				$output .= "
					<tr>
						<td><button type='button' data-id='".$row['cartid']."' class='btn btn-danger btn-flat cart_delete'><i class='fa fa-trash-alt'></i></button></td>
						...
						<td class='input-group'>
            				...
            				<input type='text' class='form-control' value='".$row['quan']."' id='qty_".$row['cartid']."'>
				            <span class='input-group-btn'>
